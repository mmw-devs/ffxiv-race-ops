...
FFXIV 高难首杀竞速网站 — 静态站点，聚合 Final Fantasy XIV 世界首杀竞速进度。展示队伍排名、Boss 血量、阶段推进、职业组合、直播链接、赛事速报。

## 命令

```bash
# 本地预览（零运行时依赖，CI 用 devDependency）
python -m http.server 8000
# 浏览器打开 http://localhost:8000

# 提交前校验 data.js（本地需先 npm ci）
node scripts/validate-data.js
```

无构建步骤、无 bundler。`package.json` 仅用于 CI（ajv schema 校验）。

## 架构

```
constants.js  ──→  index.html
schema/       ──→  scripts/validate-data.js  （CI 数据契约）
data.js       ──→  index.html                （主页面：排名、英雄区、侧边栏、滚动条）
```

**三分文件职责：**
- `constants.js` — 运行时显示常量 + 校验白名单（`PHASE_ORDER`、`ROLE_COLORS`、`VALID_REGIONS` 等）。开发者维护。
- `schema/*.json` — 6 个 JSON Schema 文件（draft-2020-12），定义 `RACE_DATA` 数据契约。开发者维护。
- `data.js` — `RACE_DATA` 纯数据值。定义全局 `RACE_DATA` 对象供 HTML 页面消费。运营侧维护，dev PR 零触碰。
...
**CSS 使用 OKLCH token 系统**，定义在每个 HTML 文件的 `:root` 中。修改 6–10 个自定义属性即可整体换肤。无共享 CSS 文件（为保持各页面自包含、零依赖）。

## 分支模型（双轨）

| 轨道 | 分支前缀 | 变更对象 | 操作者 |
|------|---------|---------|--------|
| 开发 | `feature/*`、`fix/*` | `constants.js`、`schema/`、HTML、CSS、JS、CI、docs | 开发者 |
| 运营 | `content/*` | `data.js` 数据值 | PI Agent（代表运营者） |
...
- CI 双向硬阻断：dev PR 禁含 `data.js`，content PR 仅允许 `data.js`
...
1. **所有 PR**：`npm ci` → `node scripts/validate-data.js` — 三阶段校验：
   ...
   - **值域**：与 `constants.js` 白名单交叉校验（phase ∈ `PHASE_ORDER`、region ∈ `VALID_REGIONS` 等）
   - **业务规则**：rank 连续性、`bossHP` ∈ [0,100]、每队恰好 8 人

2. **`feature/*` / `fix/*` PR**：文件范围检查 — diff **不得**含 `data.js`。违规 → 直接 FAIL。

3. **`content/*` PR**：文件范围检查 — diff **仅**允许 `data.js`。含其他文件 → 直接 FAIL。
...
- `feature/*` / `fix/*` PR 含 `data.js` → CI 阻断
- `content/*` PR 含非 `data.js` 文件 → CI 阻断
...
**阶段 2 — 值域交叉**：从 `constants.js` 取白名单，校验 `phase` ∈ `PHASE_ORDER`、`region` ∈ `VALID_REGIONS` 等
**阶段 3 — 业务规则**：
