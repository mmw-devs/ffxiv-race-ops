...
if [ -z "$APP_ID" ]; then
    ...
    echo "URL: https://github.com/organizations/ffxiv-race-stats/settings/apps/ffxiv-race-ops"
    exit 1
fi
...
REPO_INFO=$(curl -s -H "Authorization: Bearer $TOKEN" \
    -H "Accept: application/vnd.github+json" \
    "https://api.github.com/repos/ffxiv-race-stats/ffxiv-race-stats")
...
if [ "$REPO_FULL" = "ACCESS DENIED" ]; then
    echo -e "${RED}  失败: 无法访问仓库 ffxiv-race-stats/ffxiv-race-stats${NC}"
    echo "  请确认 App 已安装到该仓库:"
    echo "  https://github.com/organizations/ffxiv-race-stats/settings/installations/$INSTALLATION_ID"
    exit 1
fi
...
REPO_URL="https://x-access-token:${TOKEN}@github.com/ffxiv-race-stats/ffxiv-race-stats.git"
...
echo "  创建测试 PR..."
gh pr create \
    --repo ffxiv-race-stats/ffxiv-race-stats \
    --base main \
    --head "$TEST_BRANCH" \
