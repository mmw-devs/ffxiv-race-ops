...
if [ -z "$APP_ID" ]; then
    ...
    echo "URL: https://github.com/organizations/mmw-dev/settings/apps/ffxiv-race-ops"
    exit 1
fi
...
REPO_INFO=$(curl -s -H "Authorization: Bearer $TOKEN" \
    -H "Accept: application/vnd.github+json" \
    "https://api.github.com/repos/mmw-dev/ffxiv-race-stats")
...
if [ "$REPO_FULL" = "ACCESS DENIED" ]; then
    echo -e "${RED}  失败: 无法访问仓库 mmw-dev/ffxiv-race-stats${NC}"
    echo "  请确认 App 已安装到该仓库:"
    echo "  https://github.com/organizations/mmw-dev/settings/installations/$INSTALLATION_ID"
    exit 1
fi
...
REPO_URL="https://x-access-token:${TOKEN}@github.com/mmw-dev/ffxiv-race-stats.git"
